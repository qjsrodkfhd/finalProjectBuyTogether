$('head')
		.append(
				'<script type="text/javascript" src="/resources/js/login/loginDao.js"></script>')
...
function loginController() {
	...
	function isNewUser(userInfo, loginCheckStatus) {
		...
		var result = dao.loginSession(userInfo, loginCheckStatus); // 체크전 세션에
		// 로그인정보 저장
		...
		if (userInfo.level_number != 1) { // 관리자 로그인이라면,
			...
			if (userInfo.nickname == null) { // 첫 로그인이라면,
				 ...
				 * 
				 * alert("여기다."); alert(sessionStorage.getItem("returnUrl"));
